...
void Formula::fixedValue(int* z, unsigned value, int n)
{
	for (int i = 0; i < n; i++) {
		...
		if (debug) {
		printf("fixed: z[i]: %d | z: %d | value: %u | Varid: %d\n", z[i], *z, value, x);
		}
		addClause({ x });
	}
}
...
{
	if (debug) {
	printf("Rotate: z: %d | x: %d | p: %d | n: %d\n", *z, *x, p, n);
	for (int i = 0; i < n; i++) {
		printf("Rotate before: z[i]: %d | x[i]: %d | p: %d | n: %d\n", z[i], x[i], p, n);
		z[i] = x[(i + n - p) % n];
...
void Formula::or4(int *z, int* a, int* b, int* c, int* d, int n) {
	for (int i = 0; i < n; i++) {
		int t1[n];
		int t2[n];
		newVars(t1, n);
...
void Formula::add2(int* z, int* x, int* y, int n)
{
	if (multiAdderType == TWO_OPERAND || multiAdderType == DOT_MATRIX) {
		...
		newVars(c, n - 1);
		if (debug) {
		printf("Defaults: z: %d | x: %d | y: %d | c: %d\n", *z, *x, *y, *c);
		// printf("adder: z: %d | x: %d | y: %d | c: %d\n", *z, *x, *y, *c);
		printf("full: z: %d | x: %d | y: %d | c: %d\n", *z+1, *x+1, *y+1, *c);
		printf("xor3: z: %d | x: %d | y: %d | c: %d\n", *z+n-1, *x+n-1, *y+n-1, *c+n);
		}
		// xor2(z, x, y, 1);
		// and2(c, x, y, 1);
		halfadder(c, z, x, y, 1);
